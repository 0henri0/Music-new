                    ...
                    <div class="browse-inner">
                        ...
                        @foreach($albumAll as $album)
                            <div class="col-md-3 artist-grid">
                                <a href="{{url('albumsinger/song/'.$album->id)}}"><img class="avatar-category-list" src="{!! $album->avatar !!}" title="{!! $album->name !!}"></a>
                                <a href="{{url('albumsinger/song/'.$album->id)}}"><i class="glyphicon glyphicon-play-circle"></i></a>
                                <a class="art" href="{{url('albumsinger/song/'.$album->id)}}">{!!cutString($album->name,15)!!}</a>
                            </div>
                        @endforeach
                        {{ $albumAll->links() }}
                        <div class="clearfix"></div>
